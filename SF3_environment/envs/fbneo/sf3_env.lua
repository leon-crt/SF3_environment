...
function GameInterface()
    ...
    then
        ...
        then
            ...
            Client:settimeout(0)
            -- we need to perform busy waiting so that the emulator doesn't freak out for no reason at all
            -- also the recv call is blocking so that the environment can proceed step by step (it hardly matters when going full speed)
            while raw_p1_input == nil do
